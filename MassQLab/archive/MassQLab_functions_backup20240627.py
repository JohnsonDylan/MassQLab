        ...
        with PdfPages(data_directory + "/MassQLab_Output/"+timestr+"/ms1_traces.pdf") as pdf:
            for group_name, grouped_df in raw_df_ms1.groupby(['filename', 'query_name', 'query']):
                ...
                if len(grouped_df) >= 5:
                    ...
                    # Filter peaks based on the RT range
                    filtered_peaks = [peak for peak in peaks if (rtmin - range_rt) < xdata_all[peak] < (rtmax + range_rt)]
                    
                    # Calculate widths at half maximum for filtered peaks
                    ...
                    # Further filter peaks based on FWHM condition compared to range_rt
                    final_peaks = [peak for i, peak in enumerate(filtered_peaks) if real_width[i] < range_rt]
                    
                    # Sort the final peaks by their prominence and select the top 2
                    top_n_peaks_prominence = sorted(final_peaks, key=lambda x: ydata_all[x], reverse=True)[:2]
                    
                    # Sort the top 2 peaks based on the custom scoring function
                    ...
                    if peak_x_values.any():
                        ...
                        x_values = xdata_all
                        # interpolated_array = interpolate_array(x_values, Amidpoint[0])
                        interpolated_array = generate_linear_array(rtmin, rtmax)
                        y_values = gaussian(interpolated_array, peak_y, Amidpoint[0], sigma)
                        plt.plot(interpolated_array, y_values, label='Gaussian Peak', color='r', lw=2)
                        ...
                        plt.hlines(Awidth_heights, Aleft_ips, Aright_ips, color='blue')
                    ...
                    plt.plot(xdata_all, ydata_all, '-o', label='Data', color='black', markerfacecolor='black', markeredgecolor='black')
                    plot_symbols = ['x', '+', '+', '+', '+']
                    ...
                    pdf.savefig()
                    # plt.show()
                    if not os.path.exists(data_directory + "/MassQLab_Output/"+timestr+"/ms1_traces/"):
                        os.makedirs(data_directory + "/MassQLab_Output/"+timestr+"/ms1_traces/")
                    plt.savefig(data_directory + "/MassQLab_Output/"+timestr+"/ms1_traces/"+plt_title+'.png')
                    plt.close('all')
...
def reportlab_ms1(ms1_analysis_df, data_directory, timestr):
    ...
    if not ms1_analysis_df.empty:
        for group_name, grouped_df in ms1_analysis_df.groupby(['query_name', 'query']):
            ...
            image_path1 = data_directory + "/MassQLab_Output/"+timestr+"/ms1_summary_traces/"+group_name[0]+".png"
